/* PicoBoot LVGL config: derived from PicoDoom's trimmed lv_conf.h (LVGL v9.2.2), with the PSRAM-backed
 * memory pool replaced by LVGL's builtin static pool, and Montserrat 20 added for the title. */
/**
 * @file lv_conf.h
 * Configuration file for v9.2.2
 *
 * PicoDoom WAD-selection boot menu (Phase 4): this is LVGL v9.2.2's own
 * lv_conf_template.h with `#if 0` flipped to `#if 1` (the template's own
 * documented activation switch), carried over from TOM6809's validated
 * Phase 3 copy (same board, same library) with a small amount of
 * project-specific tuning below -- LV_MEM_SIZE + LV_MEM_POOL_ALLOC (see
 * their own comments) and the `#define` tweaks flagged inline
 * (font sizes, dark theme). Everything else is upstream defaults.
 * CMakeLists.txt points LV_CONF_PATH straight at this file (no manual
 * include-path wiring needed on our side -- LVGL's own custom.cmake adds
 * this file's directory to the `lvgl` target's PUBLIC include dirs), so it
 * is found regardless of where the fetched LVGL source lands.
 */
...
#define LV_ASSERT_HANDLER extern void picoboot_lvgl_assert(void); picoboot_lvgl_assert();
...
#define LV_USE_THEME_DEFAULT 1
    ...
    /*0: Light mode; 1: Dark mode -- PicoDoom: the DOOM-II-style boot menu is
     *dark-framed (black LCD border / black DVI canvas band around the
     *game), so the default dark palette fits without an extra contrast
     *pass. TOM6809 left this 0 (light) for its white-tile boot menu; this
     *project's menu has no such tiles.*/
 ...
 *PicoDoom: left 0 -- the boot menu only ever applies the default theme, and
 *building the handful of extra .c files these import costs flash for nothing.
 *TOM6809's copy left them on because its TOM-icon font code references them
 *via LV_USE_THEME_SIMPLE in spots; this project references neither.*/
#define LV_USE_THEME_SIMPLE 0

/*A theme designed for monochrome displays -- see LV_USE_THEME_SIMPLE.*/
